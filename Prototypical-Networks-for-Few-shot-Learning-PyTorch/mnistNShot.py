...
class MnistNShot:
    ...
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz):
        ...
        :param imgsz:
        """
        ###  (B x H x W x C). The files are not normalized.
        self.resize = imgsz
        if not os.path.isfile(os.path.join(root, 'minist.npy')):
            # TODO : May add formatting npy file here
            raise FileNotFoundError("Must have the npy file first")
        else:
            # if data.npy exists, just load it.
            self.x = np.load(os.path.join(root, 'minist.npy'))
        ...
        self.x_train = np.true_divide(self.x_train, 255)
        self.x_test = np.true_divide(self.x_test, 255)
        # self.x_train /= 255.
        # self.x_test /= 255.
...
if __name__ == '__main__':
    ...
    db = MnistNShot(r'D:\DAstudy\FastMDL-main\baselines\Prototypical-Networks-for-Few-shot-Learning-PyTorch\dataset\minist', batchsz=20, n_way=5, k_shot=5, k_query=15, imgsz=28)
